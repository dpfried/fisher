...
table_paths = [join(config.FISHER_ROOT, 'doc', 'fe_03_p%s_calldata.tbl' % disc) for disc in config.FISHER_DISCS]
...
def get_topic_info():
    ...
    return [(topic.attrib['id'], topic.attrib['title'].strip(' .'), topic.text.strip()) for topic in root]
